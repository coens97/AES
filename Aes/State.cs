...
namespace Aes
{
    public class State
    {
        ...
        private static readonly byte[] _mul3 = {
        ...
        private static readonly byte[] _mul11 = {
        ...
        private static readonly byte[] _mul13 = {
        ...
        private static readonly byte[] _mul14 = {
        ...
        public State MixColumns()
        {
            ...
            {
                s._buf[0, c] = (byte)(_mul2[_buf[0, c]] ^ _mul3[_buf[1, c]] ^      _buf[2, c]  ^      _buf[3, c]);
                s._buf[1, c] = (byte)(     _buf[0, c]  ^ _mul2[_buf[1, c]] ^ _mul3[_buf[2, c]] ^      _buf[3, c]);
                s._buf[2, c] = (byte)(     _buf[0, c]  ^      _buf[1, c]  ^ _mul2[_buf[2, c]] ^ _mul3[_buf[3, c]]);
                s._buf[3, c] = (byte)(_mul3[_buf[0, c]] ^      _buf[1, c]  ^      _buf[2, c]  ^ _mul2[_buf[3, c]]);
            }
            return (s);
        }
        ...
        public State AddRoundKey(Key key, int round)
        {
            ...
            for (var c = 0; c < NrofCol; c++)
            {
                for (var r = 0; r < NrofRow; r++)
                {
                    s._buf[r, c] = (byte)(_buf[r, c] ^ key.W[r, c + 4 * round]);
        ...
        {
            var s = "";
            for (var i = 0; i < _buf.GetLength(0); i++)
            {
                for (var j = 0; j < _buf.GetLength(1); j++)
                {
                    s += _buf[i, j].ToString("X2") + " ";
                }
                s += "\n";
            }
            return (s);
